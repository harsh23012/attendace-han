...
def is_logged_in():
    # Implement logic to check if a user is logged in (e.g., session variable)
    pass
...
def register_admin():
    if request.method == 'GET':
        ...
        return render_template('auth/register_admin.html')  # Render registration form
    else:
        name = request.form['name']
        username = request.form['username']
        password = request.form['password']

        # Validate user input (e.g., check username availability, password strength)
        ...
        try:
            ...
            if result[0] == 0:
                ...
                # Get current datetime with Asia/Manila timezone
                current_datetime = datetime.now(pytz.timezone('Asia/Manila')).strftime('%Y-%m-%d %H:%M:%S')

                # Insert new admin data
                cursor.execute("""
                    INSERT INTO users (`uuid`, `name`, `username`, `password`, `image`, `created_at`, `updated_at`) 
                    VALUES (%s, %s, %s, %s, %s, %s, %s)
                """, (user_uuid, name, username, hashed_password, 'default-user-image.png', current_datetime, current_datetime))
        ...
        return render_template('auth/register_admin.html', message=message) 
